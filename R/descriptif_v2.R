...
#
#   # Tableau descriptifs pour la moyenne d'une varibale quantitative
#   tab.res.moy <- function(x, y, weights, conf.level, decimal, mean.test, include.p){
...
#                         minmax = paste0("{", formatC(round(min(x, na.rm = TRUE), decimal), digits = decimal, format = "f"), "-",
#                                         formatC(round(max(x, na.rm = TRUE), decimal), digits = decimal, format = "f"), "}"),
...
#                          minmax = paste0("{", formatC(round(min(aux1, na.rm = TRUE), decimal), digits = decimal, format = "f"), "-",
#                                          formatC(round(max(aux1, na.rm = TRUE), decimal), digits = decimal, format = "f"), "}"),
#                          stringsAsFactors = FALSE)
...
#   tab.res.fac <- function(weights, x, y, conf.level, conf.method, decimal, simulate.p.value, chi.correct, prop.type, prop.test, include.p) {
#     if(is.defined(weights)) {
...
#                                  mean.test, conf.method, na.omit, na.str.default, na.str.value, na.num.default, na.num.value, include.p) {
...
#              mean = {return(tab.res.moy(x = x, y = y, weights = weights, conf.level = conf.level, decimal = decimal, mean.test = mean.test, include.p = include.p))},
#              med = {return(tab.res.med(x = x, y = y, conf.level = conf.level, decimal = decimal, include.p = include.p))},
#              both = {return(bind_rows( # concatenation des deux precedentes
#                tab.res.moy(x = x, y = y, weights = weights, conf.level = conf.level, decimal = decimal, mean.test = mean.test, include.p = include.p),
#                tab.res.med(x = x, y = y, conf.level = conf.level, decimal = decimal, include.p = include.p)
#              ))})
#     }
#     if(is.factor(x)){
...
#                                      mean.test = mean.test, chi.correct = chi.correct, num.type = num.type, conf.level = conf.level, conf.method = conf.method,
...
#   }
#
#   # Passage de "res" du formati liste au format data.frame
...
#   if (!include.minmax){
#     res <- res[, !str_detect(colnames(res), "minmax")]
#   } else { # Ou renommage selon les modalites de la variable facteur
#     colnames(res)[str_detect(colnames(res), "minmax")] <- c("Total minmax", paste(levels(data[, factor]), "minmax"))
#   }
#
#
...
#     colnames(res)[colnames(res) == "IC"] <- paste0("Total IC ", conf.level*100L, "%")
...
#     colnames(res)[colnames(res) == "IC"] <- paste0("Total CI ", conf.level*100L, "%")
...
#   if (is.null(pop.ref)) res <- res[, !str_detect(colnames(res), " TI")] # Suppression de la p-valeur
#
...
#   # descriptif(iris)
#   # descriptif(iris, include.minmax = TRUE)
#   # descriptif(iris, include.minmax = TRUE, include.p = TRUE)
#
# #### Test include.minmax ####
#   # descriptif(iris)
#   # descriptif(iris, include.minmax = TRUE)
#   # descriptif(iris, include.minmax = TRUE, merge.cols = FALSE)
#   # descriptif(iris, factor = "Species")
#   # descriptif(iris, factor = "Species", include.minmax = TRUE)
#   # descriptif(iris, factor = "Species", include.minmax = TRUE, merge.cols = FALSE)
#
