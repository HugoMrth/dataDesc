...
#                        conf.level = 0.95,
#                        conf.method = c("waldcc", "sisonglaz", "cplus1", "goodman", "wald", "wilson"),
#                        # Variables quanti
...
#   conf.method <- match.arg(conf.method, choices = c("waldcc", "sisonglaz", "cplus1", "goodman", "wald", "wilson"))
...
#
#   # Stops si parametres non approprie
...
#   if(is.defined(factor) & all(factor %ni% colnames(data))) stop("'factor' is not a defined column of data")
#   if(is.defined(factor) & length(factor) > 1) stop("'factor' must be unique")
#   if(is.defined(factor) & any(names(table(factor)) %in% c("Total", "range", "IC"))) {
#     err <- c("Total", "range", "IC")[which(c("Total", "range", "IC") %in% names(table(factor)))]
...
#
#   # Attribution de valeurs par defaut aux parametres non conformes
...
#     include.p <- TRUE}
#   if(include.range %ni% c(TRUE, FALSE)){
#     warning("include.range must be logical (TRUE, FALSE), default value is assigned : include.range <- FALSE")
#     include.range <- FALSE}
#
...
#   IC_med <- function (x, conf.level = 0.95, sides = c("two.sided", "left", "right"), na.rm = TRUE, conf.method = c("classic", "boot"), R = 999) {
...
#     r <- switch(match.arg(arg = conf.method, choices = c("classic", "boot")),
...
#   IC_moy <- function (x, sd = NULL, trim = 0, conf.method = c("classic", "boot"), conf.level = 0.95, sides = c("two.sided", "left", "right"), na.rm = TRUE) {
#
...
#     sides <- match.arg(sides, choices = c("two.sided", "left", "right"), several.ok = FALSE)
#     conf.method <- match.arg(conf.method, c("classic", "boot"))
...
#   }
#
#   IC_tab <- function (x, conf.level = 0.95, conf.method = c("waldcc", "sisonglaz", "cplus1", "goodman", "wald", "wilson")) {
...
#     p <- x/n
#
#     conf.method <- match.arg(arg = conf.method, choices = c("waldcc", "sisonglaz", "cplus1", "goodman", "wald", "wilson"))
#     switch(conf.method,
#            goodman = {
...
#
#   tab.res.moy <- function(x, y, weights, conf.level, decimal, cut.pvalue, p.decimal, mean.test, include.p){
...
#     temp1 <- formatC(round(IC_moy(x, conf.level = conf.level), decimal), digits = decimal, format = "f")
...
#                          range = paste0("{", min(aux1, na.rm = TRUE), "-", max(aux1, na.rm = TRUE), "}"),
#                          stringsAsFactors = FALSE)
#       colnames(aux4) <- paste0(levels(y)[kk], c("_n", "_p", "_IC", "_range"))
#       list.res[[kk]] <- aux4
#     }
...
#              })
#       if (p < cut.pvalue) {# Si la p-val est inferieure au seuil
#         p <- paste0("<", cut.pvalue) # Elle est tronquee a l'affichage
#       } else {
#         p <- formatC(round(p, p.decimal), digits = p.decimal, format = "f") # SInon elle est formate
#       }
...
#   }
#
#   tab.res.med <- function(x, y, conf.level, decimal, cut.pvalue, p.decimal, include.p) {
...
#
#     if (include.p) {
#       if (kruskal.test(x, y)$p.value < cut.pvalue) {
#         p <- paste0("<", cut.pvalue)
#       } else {
#         p <- formatC(round(kruskal.test(x, y)$p.value, p.decimal), digits = p.decimal, format = "f")
#       }
#       if(length(table(y)) == 2) {tn <- "Wilcoxson"} else {tn <- "Kruskal-Wallis"}
#     } else {
#       p <- "-"
...
#   }
#
#   tab.res.fac <- function(weights, x, y, conf.level, conf.method, decimal, simulate.p.value, chi.correct, prop.type, prop.test, include.p, cut.pvalue, p.decimal) {
#     if(is.defined(weights)) {
#       temp1 <- formatC(round(IC_tab(aggregate(w, list(x), sum)$x, conf.level = conf.level, conf.method = conf.method)*100L, decimal), digits = decimal, format = "f")
#       rownames(temp1) <- names(table(x))
#       temp2 <- formatC(round(aggregate(w, list(x), sum)$x, decimal), digits = 0L, format = "f")
#       temp3 <- formatC(round(sum(weights, na.rm = TRUE), decimal), digits = decimal, format = "f")
#     } else {
#       temp1 <- formatC(round(IC_tab(table(x), conf.level = conf.level, conf.method = conf.method)*100L, decimal), digits = decimal, format = "f")
...
#                         range = c(paste0("{", nlevels(x), " val}"), rep("", length(temp2))),
#                         stringsAsFactors = FALSE)
#     rownames(temp4) <- NULL
#     temp4$N <- as.character(temp4$N)
#     temp4$n <- as.character(temp4$n)
...
#                aux0 <- formatC(round(IC_tab((aux1[, kk]), conf.level = conf.level, conf.method = conf.method)*100L, decimal), digits = decimal, format = "f")
...
#                aux0 <- formatC(round(IC_tab(t(aux1)[, kk], conf.level = conf.level, conf.method = conf.method)*100L, decimal), digits = decimal, format = "f")
...
#              }
#              colnames(p.ic) <- unlist(lapply(levels(y), function (x) paste0(x, "_", c("p", "IC", "range"))))
#            },
#            "tot.percent" = { # Pour le total
#              aux0 <- formatC(round(IC_tab(as.vector(aux1), conf.level = conf.level, conf.method = conf.method)*100L, decimal), digits = decimal, format = "f")
...
#              colnames(aux0) <- paste0(rep(levels(y), each = 2), "_", c("p", "IC", "range"))
#              p.ic <- bind_cols(aux0)
#            })
#
...
#     ligne.vide[1, str_detect(colnames(ligne.vide), "_range")] <- # Pour les colonnes range
...
#                }
#              })
#       if (test$p.value < cut.pvalue) { # Remplacement par cut.pvalue si en dessous du seuil
#         pvalue <- paste0("<", cut.pvalue)
#       } else {
#         pvalue <- formatC(round(test$p.value, p.decimal), digits = p.decimal, format = "f")
#       }
...
#                                  mean.test, conf.method, na.omit, na.str.default, na.str.value, na.num.default, na.num.value, p.decimal, cut.pvalue, include.p) {
...
#              mean = {return(tab.res.moy(x = x, y = y, weights = weights, conf.level = conf.level, decimal = decimal, cut.pvalue = cut.pvalue, p.decimal = p.decimal, mean.test = mean.test, include.p = include.p))},
#              med = {return(tab.res.med(x = x, y = y, conf.level = conf.level, decimal = decimal, cut.pvalue = cut.pvalue, p.decimal = p.decimal, include.p = include.p))},
#              both = {return(bind_rows( # concatenation des deux precedentes
#                tab.res.moy(x = x, y = y, weights = weights, conf.level = conf.level, decimal = decimal, cut.pvalue = cut.pvalue, p.decimal = p.decimal, mean.test = mean.test, include.p = include.p),
#                tab.res.med(x = x, y = y, conf.level = conf.level, decimal = decimal, cut.pvalue = cut.pvalue, p.decimal = p.decimal, include.p = include.p)
#              ))})
#     }
#     if(is.factor(x)){
#       return(tab.res.fac(weights = weights, x = x, y = y, conf.level = conf.level, conf.method = conf.method, decimal = decimal, simulate.p.value = simulate.p.value,
#                          chi.correct = chi.correct, prop.type = prop.type, prop.test = prop.test, include.p = include.p, cut.pvalue = cut.pvalue, p.decimal = p.decimal))
#     }
#   }
#
#
#
#  # Code Fonction ----
#   # On ne garde que les variables yvarname et on transforme les donnees au bon format
#   if (is.defined(weights)) {
...
#     if (!is.null(data[, unlist(lapply(data, is.integer))])) {
...
#                                      mean.test = mean.test, chi.correct = chi.correct, num.type = num.type, conf.level = conf.level, conf.method = conf.method, p.decimal = p.decimal, cut.pvalue = cut.pvalue,
...
#                         chi.correct = FALSE, num.type = num.type, conf.level = conf.level, conf.method = conf.method,
#                         simulate.p.value = FALSE, na.omit = na.omit, na.str.default = na.str.default, na.num.default = na.num.default,
#                         na.str.value = na.str.value, na.num.value = na.num.value, include.p = include.p)
#   }
#
#   # Mise en page ####
#   if (is.null(label)) { # Nommage des elements de la liste selon (label si dispo, noms de colonnes sinon)
#     names(res) <- vars
#   } else {
#     names(res) <- label
#   }
...
#
#   order.var <- c("Var", "N", "n", "p", "IC", "range", # Definition de l'ordre des colonnes
#                  paste0(sort(factor(rep(levels(data[, factor]), 4),
...
#   if (!include.conf){ # Suppression des IC
#     res <- res[, !str_detect(colnames(res), "IC")]
#   } else { # Ou renommage selon les modalites de la variable facteur
#     colnames(res)[str_detect(colnames(res), "IC")] <- c("Total IC", paste(levels(data[, factor]), "IC"))
#   }
#   if (!include.range){ # Suppression des range
#     res <- res[, !str_detect(colnames(res), "range")]
#   } else { # Ou renommage selon les modalites de la variable facteur
#     colnames(res)[str_detect(colnames(res), "range")] <- c("Total range", paste(levels(data[, factor]), "range"))
#   }
#
...
#   }
#
#   if ((include.conf | include.range) & merge.cols){  # Gestion de la fusion des colonnes quand les IC ou les ranges sont inclus
#     ncol_suppr <- sum(c(include.conf, include.range))# By = 1 si une des deux options, 2 sinon
#     ind_c <- seq(3, ncol(res)-2, 1 + ncol_suppr)  # Seq de 3 (1 = var, 2 = N), a ncol-2 (ncol = test, ncol - 1 = pval)
...
#       if (include.range) colnames(res)[ind_c[i]] <- paste(colnames(res)[ind_c[i]], "{range}") # Idem
#     }
#     res <- res[, -as.vector(sapply(ind_c, `+`, 1:ncol_suppr))] # Suppression des colonnes concatenees
#   }
#
#   if (!merge.cols){ # Eclatage des colonnes
...
#   }
#
#   if (!include.tot) res <- res <- res[, !str_detect(colnames(res), "Total")] # Suppression du descriptif total
...
#   if (!include.p) res <- res[, colnames(res) != "pval"] # Suppression de la p-valeur
#
